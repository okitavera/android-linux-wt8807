        ...
        pr_info(LOGTAG"1. true gan");
			...
			if ((barrier[1] == true) ||
			   ((x > prevx) &&
				...
				barrier[1] = true;
#if S2W_DEBUG
        pr_info(LOGTAG"2. true gan");
#endif
				if ((x > prevx) &&
				    (y > 0)) {
					if (x > (S2W_X_MAX - S2W_X_FINAL)) {
#if S2W_DEBUG
        pr_info(LOGTAG"3. true gan");
#endif
						if (exec_count) {
#if S2W_DEBUG
        pr_info(LOGTAG"4. true gan");
#endif
							pr_info(LOGTAG"ON\n");
...
static int input_dev_filter(struct input_dev *dev) {
	if (strstr(dev->name, "touch") || strstr(dev->name, "ft5x06_ts")) {
		return 0;
	} else {
		return 1;
	}
